# Cloud Run Module for EyewearML Platform
...
resource "google_secret_manager_secret" "service_secrets" {
  for_each = {
    for pair in setproduct(keys(var.regions), keys(var.secrets)) : "${pair[0]}-${pair[1]}" => {
      region = pair[0]
      secret_key = pair[1]
      secret_value = var.secrets[pair[1]]
    }
  }
  
  secret_id = "${var.service_name}-${each.value.region}-${each.value.secret_key}"
  project   = var.project_id
  
  replication {
    user_managed {
      replicas {
        location = var.regions[each.value.region].region
      }
    }
  }
...
resource "google_secret_manager_secret_version" "service_secret_versions" {
  ...
  secret      = google_secret_manager_secret.service_secrets["${each.value.region}-${each.value.secret_key}"].id
  secret_data = each.value.secret_value
}

# VPC connector for private networking (if enabled)
resource "google_vpc_access_connector" "connector" {
  for_each = var.enable_vpc_connector ? var.regions : {}
  
  name          = "${var.service_name}-${each.key}-connector"
  region        = each.value.region
  project       = var.project_id
