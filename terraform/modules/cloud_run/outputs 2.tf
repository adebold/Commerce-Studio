...
output "vpc_connectors" {
  description = "Map of region to VPC connector"
...
output "regional_configuration" {
  ...
  value = {
    for region, config in var.regions : region => {
      ...
      vpc_connector = var.enable_vpc_connector ? google_vpc_access_connector.connector[region].name : null
    }
  }
}
...
output "compliance_status" {
  ...
  value = {
    gdpr_compliance      = var.regulatory_compliance.enable_gdpr_compliance
    healthcare_compliance = var.regulatory_compliance.enable_healthcare_compliance
    data_residency_required = var.regulatory_compliance.data_residency_required
    audit_logging_enabled = var.regulatory_compliance.audit_logging_enabled
