...
class AnalyticsShowcase {
    ...
    async initializeCharts() {
        ...
        // Live Sales Chart (Real-time Analytics page)
        if (document.getElementById('liveSalesChart')) {
            this.createLiveSalesChart();
        }
        
        // Live Customer Chart (Real-time Analytics page)
        if (document.getElementById('liveCustomerChart')) {
            this.createLiveCustomerChart();
        }
    ...
    createLiveSalesChart() {
        ...
        this.charts.set('liveSales', new Chart(ctx, {
            type: 'line',
            data: {
                labels: [],
                datasets: [{
        ...
        // Start live sales data updates
        this.updateLiveSalesChart();
    }

    createLiveCustomerChart() {
        const ctx = document.getElementById('liveCustomerChart').getContext('2d');
        
        this.charts.set('liveCustomer', new Chart(ctx, {
            type: 'doughnut',
            data: {
        ...
        // Start live customer data updates
        this.updateLiveCustomerChart();
    }
    ...
    updateLiveSalesChart() {
        const chart = this.charts.get('liveSales');
        if (!chart) return;
        ...
        const sales = 1500 + Math.random() * 3000;
        ...
        // Schedule next update
        setTimeout(() => this.updateLiveSalesChart(), 3000);
    }

    updateLiveCustomerChart() {
        const chart = this.charts.get('liveCustomer');
        if (!chart) return;

        // Generate random customer distribution
        const total = 100;
        const newCustomers = 30 + Math.random() * 30;
        const vipCustomers = 15 + Math.random() * 15;
        const returningCustomers = total - newCustomers - vipCustomers;

        chart.data.datasets[0].data = [newCustomers, returningCustomers, vipCustomers];
        chart.update('none');

        // Schedule next update
        setTimeout(() => this.updateLiveCustomerChart(), 5000);
    }
